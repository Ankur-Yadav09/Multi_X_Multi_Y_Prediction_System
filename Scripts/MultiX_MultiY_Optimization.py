...
# Configuration for DAE model
CONFIG = {
    "file_path": "Multi_X_Multi_Y_hourly_data.csv",
    "train_ratio": 0.8,          # 80% train, 20% test
    "model_path": "model_using_12_features.pth",
    "scaler_path": "scaler_using_12_features.pkl",
}
 
# 12 features - Previous feature used by ankur
# cols = [
#     'K1302_5TH_STG_DISCH_PRES',
#     '4TH_STG_SUCT_PRESS_CNTL',
#     'Overall_COT',
#     'Quench_Tower_Overhead_Temperature',
#     'Ethane_Fresh_Feed_Flow_P_T_Compensated',
#     'Ethane_Feed_Preheater_Ethane_Feed_Flow_DMCTF',
#     'K1302T_SPEED_PICKUP_1',
#     'K1301T_SPEED_PICKUP_1',
#     'CGC_Suction_Pressure'
# ]
 
# cols = [
#     'Average_Furnace_Conversion',
#     'HP_STM_FRM_FURNACE',
#     'K1302_5TH_STG_DISCH_PRES',
#     #'4TH_STG_SUCT_PRESS_CNTL',
#     'C2_Splitter_DP',
...
#     'Quench_Tower_Overhead_Temperature',
#     'FEED_TO_ERU_1',
#     'DEMETH_TOWER_REFLUX_CNTL',
#     'Demethanizer_DP',
#     'PRC_Suction_Pressure',
#     'ERC_Suction_Pressure',
...
target_cols = [#'Overall_Plant_Ultimate_Yield',
               'Ethylene_Product_Flow_Compensated',
               #'Ethylene_Loss_From_Product_To_Fuel'
               ]
# Load your daily data
df = pd.read_csv(file_path + "/" + 'Multi_X_Multi_Y_hourly_data.csv')
df["Timestamp"] = pd.to_datetime(df["Timestamp"], format="%m/%d/%Y %H:%M")

df.set_index("Timestamp",inplace =True)
df_colnames = df.columns
cols = [col for col in df.columns if col not in target_cols]
df = df[cols+target_cols]  # Select only the defined features
df.shape
 
#%% Domain filters
df = df[(df["Average_Furnace_Conversion"] > 60) & (df["Average_Furnace_Conversion"] < 68)]
#df = df[(df["Overall_Plant_Ultimate_Yield"] > 75) & (df["Overall_Plant_Ultimate_Yield"] < 80)]
